...
contract ScoreCard {
  enum Combination {
    ...
    function getScore(uint[] memory numbers) public view returns(uint){
      require(numbers.length == 5, "You must roll exactly 5 dice");

      // Sort the numbers array for easier combination checking
      uint[] memory sortedNumbers = sortNumbers(numbers);
    ...
    function isFiveOfAKind(uint[] memory sortedNumbers) private pure returns(bool) {
      return sortedNumbers[0] == sortedNumbers[4];
    }

    function isFourOfAKind(uint[] memory sortedNumbers) private pure returns(bool) {
      return (
        (sortedNumbers[0] == sortedNumbers[3]) ||
        (sortedNumbers[1] == sortedNumbers[4])
      );
    }

    function isFullHouse(uint[] memory sortedNumbers) private pure returns(bool) {
      return (
        (sortedNumbers[0] == sortedNumbers[1] && sortedNumbers[2] == sortedNumbers[4]) ||
        (sortedNumbers[0] == sortedNumbers[2] && sortedNumbers[3] == sortedNumbers[4])
      );
    }

    function isStraight(uint[] memory sortedNumbers) private pure returns(bool) {
      return (
        (sortedNumbers[0] + 1 == sortedNumbers[1] && sortedNumbers[1] + 1 == sortedNumbers[2] && sortedNumbers[2] + 1 == sortedNumbers[3] && sortedNumbers[3] + 1 == sortedNumbers[4]) ||
        (sortedNumbers[0] == 1 && sortedNumbers[1] == 2 && sortedNumbers[2] == 3 && sortedNumbers[3] == 4 && sortedNumbers[4] == 5)
      );
    }

    function isThreeOfAKind(uint[] memory sortedNumbers) private pure returns(bool) {
      return (
    ...
    function isTwoPairs(uint[] memory sortedNumbers) private pure returns(bool) {
      return (
        (sortedNumbers[0] == sortedNumbers[1] && sortedNumbers[2] == sortedNumbers[3]) ||
    ...
    function isPair(uint[] memory sortedNumbers) private pure returns(bool) {
      return (
